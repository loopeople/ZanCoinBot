...
def translate_text(text: str, target_lang: str) -> str:
    """Translate text to target_lang using deep-translator. If target_lang is 'en' or fails, return original."""
    try:
        if not target_lang or target_lang == "en":
            return text
        # deep-translator expects e.g. 'tr','es' ...
...
def normalize_lang_code(code: str) -> str:
    ...
    if code in SUPPORTED_CODES:
        return code
    # fallback to first two chars if possible
...
def build_keyboard(buttons_config, lang_code: str):
    """
    buttons_config: list of rows, each row is list of dicts:
      {"text": "Label in English", "callback": "callback_data", "url": None-or-url}
    Returns InlineKeyboardMarkup with labels translated to lang_code.
    """
    kb = []
...
def get_user_lang(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    # If user explicitly chose a language previously, use it
    lang = context.user_data.get("lang")
    if lang:
        return lang
    # Otherwise auto-detect from Telegram's language_code
...
async def send_translated(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str, buttons_config=None):
    ...
    # If callback query -> answer and edit the message
...
NO_QUESTIONS_PROMPT = "If you have no further questions, you can Register or Login, or go back."
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Do not override user chosen language here; if not set, auto-detect and store
...
async def set_language(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Acknowledge and show confirmation in chosen language
...
def main():
    ...
    # Start polling (Render: make sure no other instance uses same token)
